...
getAll(); // see the highscore first
...
function generateGame(event) {
  ...
  computer = arrComputer[Math.round(Math.random() * 2)]; //cpu picks choices
  let message;
...
async function getAll() {
  ...
  updateHighScore(data, highscoreContainer);
}

function updateHighScore(data, highscoreContainer) {
  if (data) {
